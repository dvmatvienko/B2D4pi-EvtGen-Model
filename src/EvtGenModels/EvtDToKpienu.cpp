                            ...
                            double& chi )
{
  ...
  boost.set( vp4_W.get( 0 ), -vp4_W.get( 1 ), -vp4_W.get( 2 ),
             -vp4_W.get( 3 ) );
                             ...
                             const double chi )
{
  ...
  for ( int index = 0; index < nAmps; index++ ) {
    switch ( type[index] ) {
      ...
      {
        ResonanceP( m, q, mV, mA, V_0, A1_0, A2_0, m0_1410, width0_1410, rBW,
                    amplitude_temp, delta_temp, f11, f21, f31 );
      ...
      {
        ResonanceD( m, q, mV, mA, TV_0, T1_0, T2_0, m0_1430, width0_1430, rBW,
                    amplitude_temp, delta_temp, f12, f22, f32 );
                        ...
                        EvtComplex& F10 )
{
  ...
  double delta_bg =
    atan( 2. * a_delta * pStar / ( 2. + a_delta * b_delta * pStar * pStar ) );
  delta_bg = ( delta_bg > 0 ) ? delta_bg : ( delta_bg + Pi );

  double delta_K0_1430 = atan( m_K0_1430 * width / ( m2_K0_1430 - m2 ) );
  delta_K0_1430 =
    ( delta_K0_1430 > 0 ) ? delta_K0_1430 : ( delta_K0_1430 + Pi );
